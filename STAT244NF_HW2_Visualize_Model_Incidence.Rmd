...
```{r}
ggplot(data=nipah_malaysia, aes(x=date, y=log(all_cases))) +
  geom_point() 
```
...
**Question LR 4:** We can get around the problem in LR 3 shifting all the counts by 1 so that we get all finite values for `log_all_cases`. Copy and paste your code from Question LR 2 and modify that code: `log_all_cases = log(all_cases+1)`. Now fit a linear model where the outcome is this new `log_all_cases` variable and the explanatory variable is `date`. What is the estimated growth rate for this viral outbreak? 

```{r}
nipah_malaysia <- nipah_malaysia %>%
  mutate(
    log_all_cases = log(all_cases + 1)
  )

tes <- lm(log_all_cases ~ date, data=nipah_malaysia)
summary(tes)
```

### Question LR 5:

Use the `confint()` function to determine the 95% confidence interval for the growth rate. For a complete answer, also interpret the meaning of the confidence interval in terms of the expected growth rate for Nipah virus in this population.
